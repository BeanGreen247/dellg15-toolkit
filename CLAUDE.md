...
| `tuxthrottle_prefix_relocate.py` | stdlib helper. `<appid>` moves a Steam game's `compatdata/<appid>` Proton prefix off an NTFS/exFAT drive (which can't host it — `:` in `dosdevices/c:` → `OSError [Errno 22]`, game won't start) onto the native Steam library and symlinks it back; `--check` = exit 1 if it needs moving; `--scan` = table of every prefix + status. Refuses to run as root or while Steam is running. Wired into the GUI's **Proton prefix tools** box; also standalone. |
