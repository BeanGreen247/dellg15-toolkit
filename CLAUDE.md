...
| `tuxthrottle_powerd.py` | **stdlib daemon, `import sensors`.** One poll loop: (1) closed-loop **fan curve** — `interp()` maps temp through the `powerd.json` `fan_curve.points` to an *additive* `fanN_boost`, with cool-down hysteresis; restores auto on SIGTERM/SIGINT (`FanController.restore`). (2) **AC/battery auto-switch** — on `AC*/online` transition, `apply_bundle()` sets platform_profile + a ryzenadj TDP preset (`TDP_PRESETS` mirrors the GUI). Config `~/.config/tuxthrottle/powerd.json`, re-read on mtime change. `run` = loop, `once` = single pass (for testing). Installed by the `FanCurveDaemon` tweak as a systemd unit that execs it from `{TOOLKIT_DIR}`. |
| `tuxthrottlectl.py` | **stdlib argparse CLI over `sensors.py`**, installed as `/usr/local/bin/tuxthrottlectl` by `install.sh`. `status` / `get {profile,tdp,fans,battery,nvpl,gamemode,clocks,gpumode}` / `set {profile,tdp,fan-boost,battery,nvpl,gpumode}` / `gamemode {on,off,toggle}`; `--json`; non-zero exit on failure. `set` needs root. |
