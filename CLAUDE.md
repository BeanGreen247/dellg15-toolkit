...
| `models/` | one JSON hardware profile per supported board (`g15-5515.json` = the reference), matched on DMI by `sensors.model_profile()` / `model_id()` (falls back to `g15-5515`). Fields (fan hwmon names, pwm paths, keycodes, RGB controller, battery method, `stock_ppt_w`, `tweaks_skip`) are **advisory today** — `sensors.py` still hard-codes the 5515 paths. `Item.requires_models` (`"models": [...]` in a tweak/app) hides an entry whose list doesn't include the current `model_id()`; no config entry uses it yet. See `models/README.md`. |
