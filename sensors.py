...
def read_dgpu_clock_temp_util() -> str:
    clock, temp, util = read_dgpu_values()
    if clock is None:
        return "n/a (no nvidia-smi / asleep)"
    return f"{clock} MHz, {temp} C, {util}% util"
...
def read_dgpu_values():
    ...
    try:
        out = subprocess.run(
            ["nvidia-smi", "--query-gpu=clocks.gr,temperature.gpu,utilization.gpu",
             "--format=csv,noheader,nounits"],
            capture_output=True, text=True, timeout=5,
        )
        if out.returncode != 0:
            return None, None, None
        clk, temp, util = [x.strip() for x in out.stdout.strip().split(",")]
        return int(clk), int(temp), int(util)
    except Exception:  # noqa: BLE001
        return None, None, None
